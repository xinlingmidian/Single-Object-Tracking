...
// Create a mask with same size as blob bounding box
	...
	cvDrawContours(mask, m_externalContour.GetContourPoints(), cvScalar(255, 255, 255), cvScalar(255, 255, 255), 0, CV_FILLED, 8,
		offset);
	...
	while (it != m_internalContours.end())
	{
		cvDrawContours(mask, (*it).GetContourPoints(), cvScalar(0, 0, 0), cvScalar(0, 0, 0), 0, CV_FILLED, 8,
			offset);
		it++;
	}
