...
void CalcuColorHistogram(int x0, int y0, int Wx, int Hy,
	...
	float * ColorHist, OutputArray lbp, int bins)
{
...
float CalcuWeightedPi(float rho, Mat& m, Mat& n)
{
	float pi_n, d_color2;
	double d = compareHist(m, n, CV_COMP_BHATTACHARYYA);
	float d_grad2 = d * d;
	d_color2 = 1 - rho;
	...
	unsigned char * img, int W, int H)
{
	...
	/* 计算目标模板直方图 */
	CalcuColorHistogram(x0, y0, Wx, Hy, img, W, H, ModelHist, hist_lbp, nbin);
...
void Observe(SPACESTATE * state, float * weight, int N,
	unsigned char * image, int W, int H,
	float * ObjectHist, Mat& ObjectLBP, int hbins)
{
	int i;
	float * ColorHist;
	float rho;
	...
	for (i = 0; i < N; i++)
	{
		...
		weight[i] = CalcuWeightedPi(rho, lbp, ObjectLBP);
	}
...
void ModelUpdate(SPACESTATE EstState, float * TargetHist, Mat& TargetLBP, int bins, float PiT,
	unsigned char * img, int W, int H)
{
	...
	/* (3)计算概率权重 */
	Pi_E = CalcuWeightedPi(Bha, lbp_, TargetLBP);
	...
	if (Pi_E > PiT)
	{
		for (i = 0; i < bins; i++)
		{
			...
			//TargetLBP.at<float>(i) = (float)((1.0 - ALPHA_COEFFICIENT) * TargetLBP.at<float>(i)
			//	+ ALPHA_COEFFICIENT * TargetLBP.at<float>(i));
		}
	}
	delete[] EstHist;
}
	...
	float & max_weight)
{
	...
	Hy_h = EState.Hyt;
	/* 模型更新 */
	ModelUpdate(EState, ModelHist, hist_lbp, nbin, Pi_Thres, image, W, H);
	/* 计算最大权重值 */
...
void main(int argc, char *argv[])//参数的个数  参数
{
	...
	capture.open("C:/Users/18016/Desktop/Benchmark_OTB/Project2/mmc2.mpeg");
	...
	while (1)
	{
		...
		if (!star)//初始化
		{
			...
			//himg = new unsigned char[Wid*Hei * 2];
			star = 1;
		}
		...
		if (track)
		{
			...
			{
				rectangle(frame, cvPoint(xout - WidOut, yout - HeiOut), cvPoint(xout + WidOut, yout + HeiOut), cvScalar(255, 0, 0), 2, 8, 0);
